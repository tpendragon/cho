...
RSpec.describe User, type: :model do
  ...
  describe '#populate_attributes' do
    before do
      allow(LdapDisambiguate::LdapUserAttributes).to receive(:get_groups).with(user.login).and_return(groups)
    end
  ...
  describe '#groups' do
    ...
    context "when the user hasn't updated their groups" do
      ...
      it 'returns a list of groups from the database' do
        expect(LdapDisambiguate::LdapUserAttributes).to receive(:get_groups).with(user.login).and_return(groups)
        expect(user.groups).to contain_exactly('group1', 'group2')
      end
    end
